    ...
    def run(self, planner, request_prop_names, test_object_index):
        ...
        while True:
            ...
            if planner == 'pomdp':
                ...
                action_dict = dict()
                actions = ['look', 'grasp', 'lift', 'hold', 'lower', 'drop', 'push', 'press', 'ask', 'reinit']
                for idx, action in enumerate(actions):
                    action_dict[action] = idx
                print action_dict

                action_choice_list = list()
                if s_idx == 0 :
                    action_choice_list.append(action_dict['look'])
                elif s_idx == 1:
                    action_choice_list.append(action_dict['press'])
                    action_choice_list.append(action_dict['grasp'])
                    action_choice_list.append(action_dict['push'])

                elif s_idx == 2:
                    action_choice_list.append(action_dict['lift'])
                elif s_idx == 3:
                    action_choice_list.append(action_dict['hold'])
                    action_choice_list.append(action_dict['lower'])
                elif s_idx == 4:
                    action_choice_list.append(action_dict['drop'])
                elif s_idx == 5:
                    action_choice_list.append(action_dict['reinit'])
                else:
                    print str(s_idx) + " " + s._name
                    sys.exit("wrong state!")

                a_idx = random.choice(action_choice_list)
...
def main(argv):
    ...
    cost_value = np.zeros([num_trials,9])

    reward_value = np.zeros([num_trials,9])

    accuracy_value = np.zeros([num_trials, 9])

    plan_numOfPro = list()
    for planner in ['pomdp', 'predefined', 'random']:
        for property_num in ['1','2','3']:
            plan_numOfPro.append(planner + "_" + property_num)
 
    planner_dict = dict()

    for idx, plan_pro in enumerate(plan_numOfPro):
        planner_dict[plan_pro] = idx

    # print planner_dict



    for planner in ['random']:
        for num_props in [1, 2, 3]: 
            overall_reward = 0
            overall_action_cost = 0
            success_trials = 0

            planner_key = planner + '_' + str(num_props)

            idx = int(planner_dict[planner_key])
            for i in range(num_trials): 
                ...
                print('request_prop_names: ' + str(request_prop_names))
                model = Model(0.99, request_prop_names, 0.65, -58.0, test_object_index)
                ...
                if planner == 'pomdp':
                    ...
                    appl = '/home/suhua/workspace/appl-0.96/src/pomdpsol'
                ...
                cost_value[i, idx] = action_cost

                reward_value[i, idx] = trial_reward

                accuracy_value[i, idx] = 1.0 if trial_reward - action_cost > 0 else 0.0
            ...
            data_dict['cost_' + planner_key] = cost_value[:,idx]
            data_dict['reward_' + planner_key] = reward_value[:,idx]
            data_dict['accuracy_' + planner_key] = accuracy_value[:,idx]

            df = pd.DataFrame.from_dict(data_dict)
            df.to_csv('100trials_1_test_discard_later_on.csv')
